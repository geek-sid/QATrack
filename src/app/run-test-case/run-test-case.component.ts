...
@Component({
  selector: 'app-run-test-case',
  templateUrl: './run-test-case.component.html',
  styleUrls: ['./run-test-case.component.scss']
})
export class RunTestCaseComponent {
  testCaseForm: FormGroup;
  addbuttonLabel: string = "Save";

  constructor(@Inject(MAT_DIALOG_DATA) public dialogData: any, public fb: FormBuilder, public dialogRef: MatDialogRef<RunTestCaseComponent>) {
    ...
    if (dialogData?.testObj) {
      ...
      this.testCaseForm.get("resultInformation").setValue(testObj.resultInformation);
    }
  }
